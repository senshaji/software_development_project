...
def login():
    ...
    if request.method=="POST":
        ...
        conn=connect_db()
        cursor=conn.cursor()
        cursor.execute("SELECT Ac_type,Username FROM login WHERE email = ? AND Password = ?", (email, passw))
        ...
        if record:
            if record[0] =="Admin":
                ...
                req=False
                username=record[1]
                return render_template('discover.html',admin=admin,username=username,profile=login_user,req=req)
...
def newuser():
    ...
    if request.method=="POST":
        ...
        if record:
            ...
            conn.commit()
            return render_template("newuser.html",msg="Account Created Successfully !",username=username,admin=admin,req=req,profile=login_user)
...
def apply():
    ...
    return render_template("job_application_form.html",username=username,profile=login_user,req=req,admin=admin)
...
def accecpt(email):
    ...
    conn.commit()
    return render_template ("discover.html",admin=admin,req=req,username=username)
...
def delete_ac(user):
    global username,login,admin
    conn=connect_db()
    cursor=conn.cursor()
    cursor.execute("DELETE FROM login WHERE Username=?",(user,)) 
    conn.commit()
    return render_template("dashboard.html",admin=admin,username=username)    
